...
**Atualização (ADR-40, FIND-002/FIND-007 da revisão de segurança de 2026-09-23):** o risco descrito acima era mais grave do que "órfão no bucket": como o finalize aceitava qualquer `objectName` vindo do cliente, sem checar se pertencia ao mangá do request, um usuário podia reaproveitar o `objectName` de um volume PÚBLICO (visível na URL assinada de `/reader/{id}/url`) para "finalizar" um upload em um mangá PRIVADO qualquer — e depois apagar esse volume forjado apagava o arquivo público real junto. O [ADR-40](ADR-40-objectname-vinculado-ao-manga.md) fecha isso vinculando o `objectName` ao `mangaId` e movendo o upload para um prefixo `pending/`, que também resolve o órfão de bucket (lifecycle rule de 1 dia) e limita o tamanho aceito pelo PUT (`x-goog-content-length-range`).
