...
| 19 | pdfjs-dist v4 em vez de v5 (latest) | pdfjs-dist v5 | v5 usa `Map.prototype.getOrInsertComputed`, proposta JS ainda sem suporte na maioria dos browsers. v4 é estável e cobre todos os casos do projeto. |
| 20 | Worker pdfjs servido localmente via `public/` + script `postinstall` | CDN (unpkg/jsdelivr) | unpkg bloqueia CORS em origem `http://localhost`, jsdelivr não indexa todas as versões do pacote. Worker local elimina dependência externa e garante MIME type correto via nginx customizado no container frontend. |
| 21 | `GET /reader/{volumeId}/progress` — progresso por volume | `GET /reader/progress/{mangaId}` (retorna volume mais recente do mangá) | Endpoint por mangá retornava o progresso do volume com maior número, quebrando o retorno de página ao abrir um volume anterior. Endpoint por volume é preciso e sem ambiguidade. |