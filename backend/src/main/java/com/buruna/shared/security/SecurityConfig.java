    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        return http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers("/admin/jobs/**").permitAll()
                        // Autenticação é o token OIDC verificado no controller (PubSubPushAuthenticator), não o Spring Security.
                        .requestMatchers(HttpMethod.POST, "/internal/pubsub/**").permitAll()
                        .requestMatchers(HttpMethod.POST, "/auth/register").permitAll()
                        .requestMatchers(HttpMethod.POST, "/auth/login").permitAll()
                        .requestMatchers(HttpMethod.POST, "/auth/refresh").permitAll()
                        .requestMatchers(HttpMethod.POST, "/auth/2fa/authenticate").permitAll()
                        .requestMatchers(HttpMethod.POST, "/auth/password/forgot").permitAll()
                        .requestMatchers(HttpMethod.GET, "/auth/password/reset-info").permitAll()
